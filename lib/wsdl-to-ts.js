...
function wsdlTypeToInterfaceString(d, opts = {}) {
    ...
    for (const k of Object.keys(d)) {
        ...
        if (opts.quoteProperties || (opts.quoteProperties === undefined && !/^[A-Za-z][A-Za-z0-9_-]*$/.test(k))) {
            propertyName = fast_safe_stringify_1.default(k);
        }
        let type = '';
        if (t === 'string') {
            const v = d[k];
...
function wsdl2ts(wsdlUri, opts) {
    return new Promise((resolve, reject) => {
        ...
        console.log('DESCRIPTION:', flatted_1.stringify(description));
                ...
                for (let maxi = 0; maxi < 32; maxi++) {
                    ...
                    if (regKeys0.length === regKeys1.length) {
                        ...
                        for (const rk of regKeys0) {
                            if (fast_safe_stringify_1.default(collector.registered[rk]) !== fast_safe_stringify_1.default(reg[rk])) {
                                noChange = false;
                                break;
                            }
                        }
...
function outputTypedWsdl(a, outputConfig) {
    ...
    for (const service of Object.keys(a.files)) {
        for (const port of Object.keys(a.files[service])) {
            ...
            interfaceFile.data.push(`export const ${interfaceFile.file.substring(interfaceFile.file.lastIndexOf('/') + 1)}Namespaces: string[] = ${fast_safe_stringify_1.default(a.soapNamespaces, null, 4)};`);
