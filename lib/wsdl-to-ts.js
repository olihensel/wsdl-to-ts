    ...
    for (const key of Object.keys(obj)) {
        ...
        if (t === 'string') {
            ...
            */
            ...
            if (propertyName === parentSegments.pop() &&
...
function wsdlTypeToInterfaceString(d, opts = {}) {
    ...
    for (const k of Object.keys(d)) {
        ...
        if (opts.quoteProperties || (opts.quoteProperties === undefined && !/^[A-Za-z][A-Za-z0-9_-]*$/.test(k))) {
            propertyName = (0, fast_safe_stringify_1.default)(k);
        }
        let type = '';
        if (t === 'string') {
            const v = d[k];
...
function wsdl2ts(wsdlUri, opts) {
    return new Promise((resolve, reject) => {
        ...
        console.log('DESCRIPTION:', (0, flatted_1.stringify)(description));
                ...
                for (let maxi = 0; maxi < 32; maxi++) {
                    ...
                    if (regKeys0.length === regKeys1.length) {
                        ...
                        for (const rk of regKeys0) {
                            if ((0, fast_safe_stringify_1.default)(collector.registered[rk]) !== (0, fast_safe_stringify_1.default)(reg[rk])) {
                                noChange = false;
                                break;
                            }
                        }
...
function outputTypedWsdl(a, outputConfig) {
    ...
    for (const service of Object.keys(a.files)) {
        for (const port of Object.keys(a.files[service])) {
            ...
            interfaceFile.data.push(`import type { ${types.join(', ')} } from "${template_1.TS_IMPORT_PATHS.WSDL_TYPES}";`);
            interfaceFile.data.push(`import { ArBaseSoapNode } from "${template_1.TS_IMPORT_PATHS.WSDL_TYPES}";`);
            interfaceFile.data.push(`import { XmlNamespace, XmlOrder } from "${template_1.TS_IMPORT_PATHS.WSDL_DECORATORS}";`);
            interfaceFile.data.push(`import { Type } from "class-transformer";`);
            interfaceFile.data.push(`export const ${interfaceFile.file.substring(interfaceFile.file.lastIndexOf('/') + 1)}Namespaces: string[] = ${(0, fast_safe_stringify_1.default)(a.soapNamespaces, null, 4)};`);
